...
for i = 1:length(theta0_vals)
    for j = 1:length(theta1_vals)
	  t = [theta0_vals(i); theta1_vals(j)];%构造theta
...
% Surface plot
figure;
surf(theta0_vals, theta1_vals, J_vals)%绘制三维曲面图surf(X,Y,Z) 创建一个三维曲面图。
...
contour(theta0_vals, theta1_vals, J_vals, logspace(-2, 3, 20))%contour(X,Y,Z,v)绘制等高线，这里v=logspace(a,b,n),在 10 的幂 10^a 和 10^b（10 的 N 次幂）之间生成 n 个点
%v是元素随下标单调递增的一维向量，用来设置等高线的值。即在固定的几个值的位置设置等高线。
